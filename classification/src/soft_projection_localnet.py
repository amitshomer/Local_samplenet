...
class SoftProjection(nn.Module):
    ...
    def _group_points(self, point_cloud, query_cloud, point_features=None):
        ...
        dist, idx = knn_point(group_size, point_cloud.contiguous(), query_cloud.contiguous())
        ...
        grouped_points = group_point(point_cloud.contiguous(), idx)  # B x 3 x QueryPoints x K
        grouped_features = (
    ...
    def project(self, point_cloud, query_cloud, hard=False):
        grouped_points, _ = self._group_points(point_cloud.permute(0,2,1), query_cloud.permute(0,2,1))
        grouped_points = grouped_points.permute(0,1,3,2)

        dist = self._get_distances(grouped_points, query_cloud.permute(0,2,1))
        ...
        # get weighted average of grouped_points
        weights1 = weights.repeat(1, 3, 1, 1)
        projected_points = torch.sum(
            grouped_points * weights1, dim=3).permute(0,2,1)
         # (batch_size, num_query_points, num_features)

        return projected_points, weights.permute(0,2,3,1), dist.permute(0,2,3,1)
